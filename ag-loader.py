...
def stream_response(client: OpenAI, model: str, messages: List[Dict[str, str]],
    ...
    """Streams response, optionally hiding reasoning with a spinner below."""

    console = Console(stderr=True) # 使用 stderr 输出状态，避免干扰 stdout
    ...
    def stop_status():
        ...
        if status_active:
            status.stop()
            status_active = False
            # Optional: print a final confirmation briefly if needed
            # console.print("[green]✓[/green]", end="")
            # time.sleep(0.1) # Brief pause
            # console.print("\r \r", end="") # Clear the confirmation
    ...
    try:
        ...
        for chunk in stream:
            ...
            if reasoning_content:
                if hide_reasoning:
                    start_status() # Start/keep spinner active
                else:
                    # Stop status if it was somehow active from a previous hidden block
                    stop_status()
            ...
            if content:
                # If DeepSeek reasoning was just happening, end it visually
                if is_reasoning_deepseek:
                    ...
                    if not hide_reasoning: # Only print markers if not hidden
                        ...
                        sys.stdout.flush()
                    # Spinner (status) will be stopped below if it was active
                ...
                while temp_buffer:
                    if not in_think_block_tag:
                        ...
                        if start_pos == -1: # No more <think> tags in buffer
                            # Stop spinner before printing normal content
                            stop_status()
                            ...
                            if hide_reasoning:
                                start_status() # Start spinner for the think block
                            else:
                                stop_status() # Ensure spinner is stopped
                        ...
                        if end_pos == -1: # Still inside <think> block
                            if hide_reasoning:
                                # Keep spinner running, consume buffer
                                temp_buffer = ''
                            else:
                            ...
                            if hide_reasoning:
                                # Don't stop spinner yet, might be followed by more reasoning
                                # or content which will handle stopping it.
                                pass
                            else:
                ...
                # Write the collected output for this chunk to standard output
        ...
        # --- Final Cleanup ---
        stop_status() # Ensure spinner is stopped at the end
        ...
        stop_status() # Stop spinner on interrupt
        if is_reasoning_deepseek or in_think_block_tag:
        ...
        if is_reasoning_deepseek or in_think_block_tag:
            if not hide_reasoning: sys.stdout.write(ANSI_RESET)
        sys.stdout.flush()
        console.print(f"\n[bold red]An unexpected error occurred during streaming:[/bold red] {str(e)}")
        return None
    finally:
        # Ensure spinner is definitely stopped if an error occurred before final cleanup
        # The stop_status() call handles the check internally
        stop_status()
